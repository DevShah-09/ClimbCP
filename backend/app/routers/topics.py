...
def get_topic_analytics(handle: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error generating topic analytics: {str(e)}")


@topics_router.get("/topics/{handle}/mastery", response_model=TopicMasteryResponse)
def get_topic_mastery(handle: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error generating topic mastery: {str(e)}")


@topics_router.get("/topics/{handle}/summary", response_model=TopicSummaryResponse)
def get_topic_summary(handle: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error generating topic summary: {str(e)}")


@weaknesses_router.get("/weaknesses/{handle}", response_model=WeaknessResponse)
def get_weaknesses(handle: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error generating weaknesses: {str(e)}")


@strengths_router.get("/strengths/{handle}", response_model=StrengthResponse)
def get_strengths(handle: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error generating strengths: {str(e)}")
